//! Security and depth validation tests.
//!
//! Includes TOCTOU symlink race condition detection (Unix-only)
//! and deep subcommand nesting validation (cross-platform).
...
async fn test_toctou_symlink_swap_detection() -> Result<()> {
    ...
    //
    // Genuinely Unix-only: uses std::os::unix::fs::symlink.
